# Установка и запуск на Linux (до первого запуска API)
...
**Что в итоге получится:** на машине установлены Git, Docker с Compose v2, при необходимости GPU-драйвер и NVIDIA Container Toolkit; вы клонировали репозиторий, выполнили `docker compose up --build`, дождались загрузки модели Ollama и получили ответы от `GET /health` и тестового `POST /generate`.
