# Полная установка и запуск на Windows + WSL с нуля
...
**Что в итоге получится:** установлены WSL2 и Docker Desktop, при наличии видеокарты NVIDIA настроен GPU в WSL; репозиторий лежит в файловой системе WSL; выполнены `docker compose up --build`, проверены `curl` на порты **8080** и **11434**, выполнен тестовый запрос к `/generate`.
