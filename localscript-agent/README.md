...
- **Сервер:** один раз **`run_all_checks`** по переданному **`code`** → в ответе **`checks`**, **`all_checks_passed`**; затем один вызов LLM (отдельный system prompt, не как у `/generate`). В ответе **`problem_description`**, **`suggested_code`** (черновик от модели; может не проходить проверки).
...
| `/help` | Справка |
| `/health` | `GET /health` |
| `/settings` | Текущие настройки CLI |
...
| `/refine` | Уточнение последнего кода (`refinement_history` собирается автоматически) |
